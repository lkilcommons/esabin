...
src_hd5f_local_dir,src_hd5f_filename = os.path.split(args.src_h5fn)
src_esagrid_file = EsagridFile(src_hd5f_filename,
                                hdf5_local_dir=src_hd5f_local_dir)

if os.path.exists(args.dest_h5fn):
    if not args.append:
        raise IOError(('Destination file {} exists'.format(args.dest_h5fn))
                      +' (--append to add to this file)')
    dest_hd5f_local_dir,dest_hd5f_filename = os.path.split(args.dest_h5fn)
    dest_esagrid_file = EsagridFile(dest_hd5f_filename,
                                    hdf5_local_dir=dest_hd5f_local_dir)
